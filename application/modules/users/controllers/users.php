 	...
 	*/
 	...
 	{	
 		...
 		if($_POST)
 		{
 			...
 			$data = array('username'=>$email,'type'=>$type);
 			$query = $this->mdl_users->get_where_array($data);

 			if(count($query->result()) > 0)
 			{
 				$result = array(' success'=> true, 'message' => 'Email already taken' );
 			}
 			elseif(count($query->result()) < 1)
 			{
 				$result = array( 'success' => false , 'message'=> 'Email not available' );
 			}
 	...
 	public function register( $usertype, $data = NULL)
 	{
		...
		switch ($usertype) {
			...
			case 'basic':
				...
				$status = $this->email->send_email('local',$this->input->post('email'), strtolower(trim($from_name)), 'FlipStop User Account', $message, 'registration');
				return $status;
 	...
 	public  function resend_activation()
 	{
 		...
 		if($_POST)
 		{
	        ...
	        $from_name = 'yourfriends@myflipstop.com';
			$status = $this->email->send_email( HTTP_ROUTE_TYPE ,$this->input->post('email'), strtolower(trim($from_name)), 'FlipStop User Account', $message, 'resend_activation_link');
			return $status;
 	...
 	public  function reset_password()	
 	{
		...
		if($_POST){
			...
			$this->mdl_users->_update_where('username', $email, array( 'password' => md5( $password ) ) );
			...
			$from_name = 'yourfriends@myflipstop.com';
       		$status = $this->email->send_email(HTTP_ROUTE_TYPE,strtolower($email), strtolower(trim($from_name)), 'Flipstop - Password Reset', $message, 'forgot_password'); 
       		return $status;
